...
import { wholesaleSchema } from '@/lib/validators';
...
function buildWholesaleHtml(data: {
  nombre: string;
...
export async function POST(request: NextRequest) {
  ...
  if (!resend) {
    ...
    console.log('[mayoristas] RESEND_API_KEY no configurado. Datos recibidos:', data);
  ...
  try {
    await resend.emails.send({
      ...
      subject: `Nueva solicitud de mayorista — ${data.nombreNegocio}`,
      html: buildWholesaleHtml(data),
    });
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error('[mayoristas] Error al enviar email:', err);
